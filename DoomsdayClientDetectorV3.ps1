...
"@
    ...
    Write-Host ""
    Write-Host "                    TeslaPro Client Scanner v1.2 (USN Journal)" -ForegroundColor Cyan
    Write-Host ""
}
...
function Start-TeslaProScan {
    ...
    if ($allJarPaths.Count -eq 0) {
        ...
        Write-Host "    .\DoomsdayClientDetectorV3.ps1 -Debug" -ForegroundColor White
        return
    }
    ...
    Write-Host "[*] Scanning files for TeslaPro Client..." -ForegroundColor Cyan
    Write-Host ""
    ...
    foreach ($assumedPath in $existingPaths.Keys) {
        ...
        Write-Progress -Activity "Scanning for TeslaPro Client" `
        ...
        try {
            $result = Test-TeslaProClient -Path $actualPath
    ...
    Write-Progress -Activity "Scanning for TeslaPro Client" -Completed
    Write-Host ""
    ...
    Write-Host "TeslaPro Client detections: " -NoNewline
    ...
    if ($detections.Count -gt 0) {
        ...
        Write-Host ""
        Write-Host "TESLAPRO CLIENT DETECTED ON THIS SYSTEM!" -ForegroundColor Red
        ...
        Write-Host ""
        Write-Host "No TeslaPro Client detected!" -ForegroundColor Green
    }
...
# Run the scan
# To enable debug mode, use: Start-TeslaProScan -Debug
Start-TeslaProScan
