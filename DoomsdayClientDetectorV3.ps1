...
function Get-RecentDeletionsFromUSN {
    ...
    foreach ($driveLetter in $DriveLetters) {
        try {
            ...
            if (-not $usnOutput -or $usnOutput.Count -eq 0) {
                if ($script:DebugMode) {
...
function Show-TeslaSummaryTab {
    ...
    if ($high -gt 0) { Write-TeslaStat -Label "HIGH" -Value "$high" -ValueColor Red } else { Write-TeslaStat -Label "HIGH" -Value "0" -ValueColor Green }
    if ($medium -gt 0) { Write-TeslaStat -Label "MEDIUM" -Value "$medium" -ValueColor Yellow } else { Write-TeslaStat -Label "MEDIUM" -Value "0" -ValueColor Green }
    if ($low -gt 0) { Write-TeslaStat -Label "LOW" -Value "$low" -ValueColor Gray } else { Write-TeslaStat -Label "LOW" -Value "0" -ValueColor Green }
}
...
function Start-DoomsdayScan {
    ...
    foreach ($path in $uniquePaths) {
        ...
        if ($foundPath) {
            $fileItem = Get-Item $foundPath -ErrorAction SilentlyContinue
            if ($fileItem) {
    ...
    if ($missingCount -gt 0) {
        ...
        foreach ($missingPath in $trulyMissingPaths) {
            ...
            if ($missingPath -notmatch '\.(JAR|EXE|DLL|BIN|DAT)$') {
                continue
            }
            ...
            $selfDestructResult = Test-SelfDestructSuspicion -Path $missingPath -SourceFile $sourcePf -RecentDeletion $recentDeletion
            ...
            $deletedFilesForUi += [PSCustomObject]@{
                ...
                Reason              = $reason
                Suspicious          = $selfDestructResult.IsSuspicious
                SuspicionConfidence = $selfDestructResult.Confidence
                SuspicionScore      = $selfDestructResult.Score
                SuspicionReasons    = ($selfDestructResult.Reasons -join " | ")
            }

            if ($selfDestructResult.IsSuspicious) {
                $selfDestructFindings += [PSCustomObject]@{
                    Path         = $missingPath
                    SourceFile   = $sourcePf
                    Confidence   = $selfDestructResult.Confidence
                    Score        = $selfDestructResult.Score
                    DeletionTime = if ($deletionTime) { $deletionTime.ToString("yyyy-MM-dd HH:mm:ss") } else { $null }
                    Reason       = $reason
                    Reasons      = $selfDestructResult.Reasons
                }
            }
            ...
            Write-Host $sourcePf -ForegroundColor Cyan
            ...
            if ($selfDestructResult.IsSuspicious) {
                Write-Host "      Self-Destruct Suspicion: " -NoNewline
                switch ($selfDestructResult.Confidence) {
                    ...
                    default  { Write-Host $selfDestructResult.Confidence -ForegroundColor White }
                }

                Write-Host "      Suspicion Score: " -NoNewline
                Write-Host $selfDestructResult.Score -ForegroundColor White
            }
        }
    ...
    if ($existingPaths.Count -eq 0) {
        ...
        Show-TeslaDashboard -Stats $stats -Detections @() -DeletedFiles $deletedFilesForUi -SelfDestructFindings $selfDestructFindings
        return
    }
        ...
        try {
            ...
            if ($result.IsDetected) {
                ...
                switch ($result.Confidence) {
                    ...
                    "MEDIUM" { Write-Host "MEDIUM" -ForegroundColor Yellow }
                    "LOW"    { Write-Host "LOW" -ForegroundColor Gray }
                    default  { Write-Host $result.Confidence -ForegroundColor White }
                }
    ...
    Show-TeslaDashboard -Stats $stats -Detections $detections -DeletedFiles $deletedFilesForUi -SelfDestructFindings $selfDestructFindings
